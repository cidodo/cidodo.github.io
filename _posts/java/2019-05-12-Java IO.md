...
layout:     post
title:      Java I/O
subtitle:   Java IO、BIO、NIO、AIO、Netty
date:       2019-05-12
author:     Arain
...
---
# Java I/O处理
Java IO流总共有40多个类，都是从如下四个基类派生出来的：
- InputStream/Reader：所有输入流的基类，前者是字节输入流，后者是字符输入流；
- OutputStream/Writer：所有输出流的基类，前者是字节输出流，后者是字符输出流。  

为什么Java中会有两种IO流？对于IO而言不管是文件读取还是网络数据，信息的最小存储单元都是字节，那么为什么还需要字符类型的IO呢？  
字符流是由Java虚拟机将字节转换得到的，问题就在于这个过程非常耗时，并且，如果我们不知道编码类型的话就很容易乱码。所以，IO流就直接提供了一种可以操作字符的接口，方便开发者平时对字符进行流操作。 
按照操作方式可将常见的IO流做以下划分：
- Java IO
    - Reader-字符读取
        - 节点流
...
## BIO、NIO和AIO
Java 中的 BIO、NIO和 AIO 理解为是 Java 语言对系统的各种 IO 模型的封装。程序员在使用这些 API 候，不需要关心操作系统层面的知识，也不需要根据不作系统编写不同的代码。只需要使用Java的API就可以了。
